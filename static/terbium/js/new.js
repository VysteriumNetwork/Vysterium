...
function ser() { 
    ...
    window.open(indexing$config.prefix + xor.encode(url), "_self");
}
...
if (urltoOpen == null) {
    siteFrame.setAttribute("src", `https://${parentDomain}' + indexing$config.prefix + ${xor.encode(urlToOpen)}`);
...
if (urlToOpen != "undefined") {
    ...
    if(parentDomain === "localhost") {
        siteFrame.setAttribute("src", `//${parentDomain}:${window.parent.document.location.port}`  + indexing$config.prefix + `${xor.encode(urlToOpen)}`);
        urlbar.value = urlToOpen;
    }
}