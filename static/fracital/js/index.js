...
function go(link) {
  ...
  document.getElementById("frame").src=indexing$config.prefix+xor.encode(link);
};
...
let observer = new MutationObserver(mutations => {
  mutations.forEach(mutation => {
	...
	if(!document.getElementById("urlbar").src == "") {
      document.getElementById("urlbar").src=xor.decode(document.getElementById("frame").contentWindow.location.href.split(indexing$config.prefix)[1]);
    }
   };
  });
});
