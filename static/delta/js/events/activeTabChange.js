...
function activeTabChange (data) {
  ...
  if (iframe.src != location.protocol + "//" + location.hostname +'/newtab.html' && iframe.src.indexOf(indexing$config.prefix) > -1){
    // let link = _browser_.decodeUrl(iframe.contentWindow.location.href.split("/service/")[1])
  adr.innerText = _browser_.decodeUrl(iframe.contentWindow.location.href.split(indexing$config.prefix)[1])
