...
function tabAdd (data) {
  ...
  const loop = setInterval(() => {
    // parse url
    const newUrl = _browser_.decodeUrl(iframe.contentWindow.location.href.split(indexing$config.prefix)[1]);
    ...
    try {
      ...
      let link = _browser_.decodeUrl(iframe.contentWindow.location.href.split(indexing$config.prefix)[1]); 
